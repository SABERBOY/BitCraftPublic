...
#[spacetimedb::reducer]
#[feature_gate]
pub fn dropped_inventory_release(ctx: &ReducerContext, dropped_inventory_entity_id: u64) -> Result<(), String> {
    ...
    let mut dropped_inventory = unwrap_or_err!(
    ...
    if dropped_inventory.owner_entity_id != actor_id {
        return Err("You are not the owner of this dropped inventory".into());
    }
